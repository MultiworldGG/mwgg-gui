...
class MultiMDApp(LiveForwarding, MDApp, metaclass=LiveTitleMeta):
    ...
    def create_custom_screen(self, title: str, content=None, index: int = -1):
        ...
        * ``create_custom_screen(title, content, index)`` -- per-world use
        ...
        icon = (getattr(content, "nav_icon", None) or ClientTab.icon) # or live._world_tab_icon() (not sure of legality using their icons)
    ...
    def open_top_appbar_menu(self, menu_button):
        ...
        if self.role == ROLE_LAUNCHER:
            ...
            if self.screen_manager.current != "launcher":
                self.change_screen("launcher")
